...
class NiceHash {
  ...
  async getBalance() {
    ...
    try {
      let time = await this.getTime();
      let balances = await this.get('/main/api/v2/accounting/accounts', {
        query
      });
  ...
  */
  ...
  async getOrders(algo) {
    ...
    let market = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'EU';
  ...
  async createOrder(options) {
    ...
    try {
      ...
      const res = await this.post('/main/api/v2/hashpower/order', {
        body
      });
      return res;
    } catch (err) {
